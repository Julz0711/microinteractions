...
export function Temperature(props: TemperatureProps) {
  ...
  return (
    <div className={twMerge("relative w-full temperature", props.styles)}>
      ...
      <div
        className={twMerge(
          isOn ? "text-light drop-shadow-[0_0_10px_rgba(0,0,0,.3)]" : props.size && "text-dark",
      ...
      >
        ...
        <span className={twMerge(
          ...
          "text-sm opacity-50 drop-shadow-[0_5px_15px_rgba(0,0,0,1)]", props.size&& "h-4")}>
      ...
      >
        <defs>
          <radialGradient id="gradientStroke" cx="40%" cy="5%" r="60%" fx="50%" fy="50%">
          <stop offset="30%" style={{ stopColor: "#EF8555", stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: "#e5527f", stopOpacity: 1 }} />
          </radialGradient>
        </defs>
         <circle
          cx={size / 2}
          cy={size / 2}
          r={(250 / 2)}
          className={twMerge("bg", props.size && isOn ? "stroke-light" : "stroke-dark")}
        ></circle>
        <circle
          cx={size / 2}
          cy={size / 2}
          r={(250 / 2)}
          className={twMerge("fg stroke-dark", props.size ? "duration-300" : "stroke-light")}
